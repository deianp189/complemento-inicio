...
\begin{lstlisting}[style=commandstyle, caption={Descargando e instalando las reglas Community Rules}, label={cod:descargar-reglas}]
...
Posteriormente, editamos el archivo \texttt{snort.lua} \cite{snort_user_manual}, ubicado en \path{/usr/local/snort/etc/snort/}, para incluir la ruta del archivo \texttt{community.rules}. Esto permite que Snort cargue automáticamente estas reglas al iniciar y pueda utilizarlas para detectar patrones maliciosos en el tráfico de red. La edición del archivo puede realizarse con herramientas como \texttt{nano}, \texttt{vim} o cualquier otro editor de texto de preferencia. El proceso se muestra en el Código~\ref{cod:editar-snort-lua}.

\begin{lstlisting}[style=commandstyle, caption={Editando el archivo snort.lua para incluir reglas personalizadas}, label={cod:editar-snort-lua}]
...
\begin{lstlisting}[style=commandstyle, caption={Validación de la configuración final de Snort con reglas}, label={cod:validar-config}]
...
Además de la configuración general mostrada en la Figura~\ref{fig:http-inspect}, es necesario agregar al final de la sección de \texttt{binders} el bloque representado en la Figura~\ref{fig:binders-http} para asegurar su correcto funcionamiento.\newline

Para finalizar, validamos la configuración de forma avanzada, como se muestra en el Código~\ref{cod:validacion-avanzada}.

\begin{lstlisting}[style=commandstyle, caption={Validación avanzada de la configuración de Snort}, label={cod:validacion-avanzada}]
...
El siguiente módulo que habilitaremos será el \textbf{SSL Inspector}. Su configuración se realiza como muestra la Figura~\ref{fig:ssl-config}.
...
Luego, se debe vincular el módulo con los binders como se muestra en la Figura~\ref{fig:ssl-binders}.
...
Finalmente, validamos y reiniciamos el servicio como en el Código~\ref{cod:validar-reiniciar}.

\begin{lstlisting}[style=commandstyle, caption={Validación de la configuración y reinicio del servicio Snort}, label={cod:validar-reiniciar}]
...
La configuración del preprocesador Stream IP se muestra en la Figura~\ref{fig:stream-ip}.
...
Este módulo, sucesor de \texttt{Stream5}, se configura como muestra la Figura~\ref{fig:stream-tcp}.
...
El preprocesador Reputation se configura como se muestra en la Figura~\ref{fig:reputation}.
...
Creamos un archivo \texttt{custom.rules} para definir reglas específicas, como se ve en el Código~\ref{cod:custom-rules} y la Figura~\ref{fig:sensitive-rules}.

\begin{lstlisting}[style=commandstyle, caption={Editando el archivo custom.rules para incluir reglas personalizadas}, label={cod:custom-rules}]
...
\begin{figure}[H]
	...
	\label{fig:agregar-customrules}
\end{figure}

\subsubsection*{Antivirus ClamAV}

Se instala ClamAV según el Código~\ref{cod:clamav-install}, y se actualizan las firmas como se ve en la Figura~\ref{fig:clamav1} y Figura~\ref{fig:clamav2}.

\begin{lstlisting}[style=commandstyle, caption={Instalación del antivirus ClamAV}, label={cod:clamav-install}]
...
\begin{lstlisting}[style=commandstyle, caption={Actualización de firmas y reinicio del servicio ClamAV}, label={cod:clamav-update}]
...
\textbf{Metodología:}  
Se han lanzado múltiples flujos simultáneos desde el servidor Ubuntu (\texttt{192.168.1.133}) hacia la Raspberry Pi (\texttt{192.168.1.132}), utilizando la herramienta \texttt{iperf3} en modo UDP. Para maximizar la presión sobre el sistema, se han abierto varias sesiones en paralelo y sin limitación de pacing, aumentando así la congestión de red y la carga sobre la CPU y la memoria del sistema.
